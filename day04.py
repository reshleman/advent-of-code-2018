...
def slice_before(iterable, predicate):
    """
    Chunk `iterable` before each element that satisfies `predicate`.

    `predicate` is not evaluated for the first element in `iterable`.

    >>> list(slice_before([0, 1, 2, 3, 4, 5, 6, 7], lambda x: x % 2))
    [[0], [1, 2], [3, 4], [5, 6], [7]]

    >>> list(slice_before('abcdefghijklmnopqrstuvwxyz', lambda x: x in 'aeiou'))
    [['a', 'b', 'c', 'd'],
     ['e', 'f', 'g', 'h'],
     ['i', 'j', 'k', 'l', 'm', 'n'],
     ['o', 'p', 'q', 'r', 's', 't'],
    ...
    https://ruby-doc.org/core-2.2.0/Enumerable.html#method-i-slice_before
    """
...
def _sorted_entries():
    return sorted({
        datetime.datetime.strptime(timestamp, '%Y-%m-%d %H:%M'): description
        for line
        in input_
        for timestamp, description
        in (re.compile(r'\[(.+)\] (.*)').match(line).groups(),)
    }.items())


def _build_sleep_map():
    # {guard_id: {minute: asleep_frequency}}
    guards = collections.defaultdict(lambda: collections.defaultdict(int))

    for entries in slice_before(_sorted_entries(), lambda x: x[1].startswith('Guard')):
        guard_number = int(re.search(r'\d+', entries.pop(0)[1]).group())

        while True:
            try:
                sleep_time, _ = entries.pop(0)
            except IndexError:
                break

            wake_time, _ = entries.pop(0)

            for minute in range(sleep_time.minute, wake_time.minute):
                guards[guard_number][minute] += 1

    return guards


def part1():
    guards = _build_sleep_map()

    guard_total_sleep_time = {
        guard_id: sum(minutes.values())
        for guard_id, minutes
        in guards.items()
    }

    sleepiest_guard_id = max(guard_total_sleep_time, key=guard_total_sleep_time.get)
    sleepiest_minute = max(guards[sleepiest_guard_id], key=guards[sleepiest_guard_id].get)
...
def part2():
    ...
    for guard_id, minutes in guards.items():
        for minute, asleep_frequency in minutes.items():
            if asleep_frequency > max_frequency:
                max_frequency = asleep_frequency
